...
- **复合评树**：Kalman 置信度与 WS 趋势的综合加权

总分 ≥ 0 通过（不设硬性门槛）。Probe 试探同步使用五树投票，高分进程探测间隔缩短，纯加速不拦截。
...
- **校准度**：对比 Kalman 预测与实际释放。偏差过大时重置卡尔曼参数并降低置信；偏差极小时奖励置信
...
**清理操作**：6 种操作独立开关：ws（进程工作集）、standby（备用列表）、modified（已修改页）、compress（内存压缩）、volume（卷缓存）、registry（注册表缓存）。默认全部开启。
