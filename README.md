...
## Windows Intelligent Memory Custodian
...
系统的核心价值在于"主动+持续"：在 Windows 自身内存压力感知机制启动之前提前介入回收，并在守护模式下保持 60 秒间隔内零空闲的持续优化。同时通过 Thompson Sampling、Kalman 滤波、情景记忆、五树投票等学习与决策机制，为每个进程建立独立画像，在最大化释放效率的同时抑制缺页副作用。

*MemWise intercepts memory pressure before Windows initiates its own reclamation, maintaining uninterrupted optimization at 60-second intervals in daemon mode. A cognitive engine combining Thompson Sampling, Kalman filtering, episodic memory, and five-tree policy voting builds independent behavioral profiles per process, maximizing release efficiency while minimizing page-fault side effects.*
...
## 2. Three-Layer Cleaning Engine

### 2.1 Layer 1 — System-Level Kernel Reclamation
...
### 2.2 Layer 2 — Per-Process Idle Page Reclamation
...
1. **安全过滤 · Safety Gate**：排除系统核心进程、前台窗口保护（压力达到阈值时放开）、工作集过小的微小进程、WS 基线检查（上一轮清理后 WS 无增长则不重复清理）、失败冷却检查

2. **策略投票（五树决策）· Five-Tree Policy Vote**：Kalman 预测收益、历史记忆验证、时序活跃度判断、Kalman 反事实优势、复合评分加权。总分 ≥ 0 即通过（不设硬性门槛）

3. **复合评分排序 · Composite Score Ranking**：θ、Kalman 预期释放量、WS 大小、WS 回弹率四维加权，EFIS 可调权重

4. **PID 增益调度 · PID Gain Scheduling**：根据当前内存压力自动分三区调节响应强度——低压（<40%）保守、中压（40–75%）标准、高压（>75%）激进，避免单一参数组在不同场景下顾此失彼

5. **并行执行 · Parallel Execution**：4 线程池，按评分降序提交

6. **自适应 Pass 数 · Adaptive Pass Count**：大进程（WS>200 MB 或 θ 超阈值）→ 4 pass；默认 → 3 pass；低 θ → 1 pass

7. **PF 反馈（先收后审）· PF Feedback (Collect-First, Judge-Later)**：对比清理前后缺页计数。释放量无条件计入统计，缺页仅用作 Thompson 学习信号的正负向判定

### 2.3 Layer 3 — Deep Iterative Reclamation
...
### 2.4 Continuous Optimization Architecture
...
- 时间衰减：长时间未观测的估计值协方差异步膨胀，保证重新遇到该进程时能快速收敛
- 观测噪声纳入 EFIS 自动调参范围，系统可根据实际预测误差自动寻优

### 3.3 Episodic Memory

存储每次清理与试探的完整上下文 episode，包含时间戳、进程标识、工作集大小、系统内存占用率、动作类型、结果与释放量等十余项字段。以定长双端队列滚动存储。检索时使用加权相似度评分，综合考量内存压力相似度、同名进程加成和时间衰减。支持按时间窗口查询同名进程的历史成功率。新进程通过分层先验获得同类进程的平均初始倾向，加速冷启动。

*Each cleanup or probe stores a full-context episode with over a dozen fields — timestamp, process identity, working set size, system memory percentage, action type, outcome, and freed bytes. A fixed-length deque provides bounded rolling storage. Retrieval uses weighted similarity scoring that combines memory-pressure proximity, same-name affinity, and temporal decay. Time-windowed historical success rates can be queried per process name. New processes receive an initial θ inherited from their category's hierarchical prior, accelerating cold-start convergence.*

### 3.4 Hierarchical Prior
...
*A comprehensive diagnostic cycle runs every 30 seconds:*

- **校准度 · Calibration**：对比 Kalman 预测与实际释放。偏差越大衰减越多——线性增大 Kalman 不确定性和缩归 Thompson 参数，替代旧版一次性重置
...
## 5. EFIS v3 — System-Wide Intelligent Parameter Tuning
...
通过进程名匹配（内置已知游戏清单 + 用户自定义名单）和全屏窗口检测（含类名过滤排除浏览器等伪全屏窗口）自动激活。激活后非前台进程的清理门槛大幅降低，前台游戏进程加强保护，最大化游戏可用内存。

*Auto-activated via process-name matching against a built-in catalog and user-defined list, combined with full-screen window detection that filters out browser and presentation pseudo-fullscreen windows. When active, non-foreground cleanup thresholds are significantly lowered while the foreground game process gains enhanced protection.*

---

## 8. Charting & Efficiency Scoring
