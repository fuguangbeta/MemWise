...
**上下文特征修正** — 基础 θ 经过 5 维特征（归一化 WS、波动率、PF 成本/收益比、置信度、偏置）的 sigmoid 加权修正，消除大/小进程之间的尺度偏差。

**PID 连续反馈控制器** — 以 45% 为目标内存使用率，P/I/D 三项连续调节清理强度 aggressiveness（0~1）。积分项消除稳态偏差，微分项抑制震荡。强度 >0.8 时 tick 间隔缩至 10s，<40% 时放宽至 60s。

**ERIS v2 效率评分系统** — 五维几何平均（吞吐/自适应/精准/动量/上下文），各维度最低 0.1 保底。完整区分“休息期（系统干净无操作，高分）”和“故障期（有操作但全失败，低分）”，差值约 38%。

### 认知引擎（新增 6 模块）

- **Kalman 滤波** — 对每个进程做二维卡尔曼追踪（预期释放量 + PF 代价），自适应过程噪声 q。新息大→加速跟踪，新息小→稳定滤波
- **情景记忆** — 存储清理时刻的五维上下文向量，余弦相似度检索 top-3 最相似历史经验
- **分层先验** — 10 类进程各自维护经验池，新进程从同类继承 θ 初始值
- **因果推理** — 有向图记录“清 A 时 B 的释放量”，支持反事实优势比查询
- **五树投票决策** — 收益/代价/时机/紧迫/反事实五棵决策树综合输出 should_trim/should_probe
- **元认知自我监控** — 每 30s 运行五维诊断（校准度/概念漂移/探索覆盖/后悔度/系统操作监控），输出日志到 GUI

### 自适应调参层

**EFIS 效率反馈智能系统** — 每 30 tick 运行诊断→调参→评估闭环。支持四场景独立参数、历史最优回归、自动回滚与方向冷却。独立文件 efis_state.json 存储。

### 基础设施

**线程安全架构** — 完全重写的消息队列架构（queue.Queue + _poll_msg_queue），消除 8 处非线程安全的 root.after() 调用。

**Motion 式图表交互** — 单 <Motion> 绑定替代 N 对 <Enter>/<Leave> 柱条事件绑定。数据回填机制确保图表折点与底部效率文字同源同值。

**零外部依赖** — 纯 ctypes Win32 API，不依赖 pywin32、psutil、numpy 等任何第三方库。
