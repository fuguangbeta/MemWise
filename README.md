...
Windows 内存整理工具，纯 ctypes Win32 API，零外部依赖。
**不杀进程、不挂起线程、不注入、不联网。** 托盘静默运行。

v1.4 引入完整的认知引擎：Kalman 滤波追踪、情景记忆检索、分层先验经验、因果推理、五树投票决策、EFIS 自适应调参、MetaCognition 元认知监控。从“跑算法的工具”升级为“真正在学习的系统”。
