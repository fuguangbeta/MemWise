...
## 智能内存看护 · 认知架构升级 · 自学习清理引擎

Windows 智能内存整理与看护工具。纯 ctypes Win32 API，零外部依赖，单 exe 分发。
**不杀进程、不挂起线程、不注入、不联网。** 托盘静默运行，Ctrl+Shift+M 热键呼出。

v1.4 引入完整**认知引擎六件套**，从“跑算法的工具”升级为“真正在学习的内存管家”：

- **Kalman 滤波** — 对每个进程追踪预期释放量与 PF 代价，支持连续反馈，取代 Beta 二值
- **情景记忆** — 存储每次清理的上下文向量，工况相似时复用历史经验，加速冷启动收敛
- **分层先验** — 10 类进程（浏览器/开发工具/游戏/系统…）各自维护同类经验，新进程继承同类 θ 先验
- **因果推理** — 记录进程间清理的因果影响（清 A 后 B 释放 50MB），支持反事实查询与排序优化
- **五树投票** — 收益/代价/时机/紧迫/反事实五棵决策树综合投票，替代单一 θ 阈值门控
- **元认知** — 校准度监控（Kalman 预测 vs 实际）、概念漂移检测（EWMA 快慢速比）、探索覆盖评估，每 30s 自动诊断输出日志

加上 **EFIS 效率反馈智能系统**（实验日志 + 自动回滚 + 场景记忆 + 自适应步长）、**ERIS v2 五维几何平均效率评分**、完全重写的线程安全消息队列架构、Motion 式图表悬浮 UI，以及数 10 项深度修复。

累计修改 50+ 处代码，涉及全部 18 个源文件，新增 8 个核心模块。
