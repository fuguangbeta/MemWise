...
图表数据源为统一的释放量累加器——所有操作的释放量统一汇入，通过累计差值法计算每轮增量，不依赖惰性更新的系统 API。日志、统计栏、图表三者同源一致。X 轴显示最近的轮次数据，折线为 ERIS v5 效率评分（0–100%+，上不封顶）。ERIS 以 EWMA 自适应基线归一化五个维度——预测精准度、参数收敛度、副作用控制、学习收敛度、探索完备度——每个维度的自身 EWMA 衰减基线为 100% 基准线，比值经 soft sigmoid 压缩（渐近上限 3.0）后作为维度评分，自适应 EWMA 根据偏离幅度动态调整收敛速度。释放量不参与分数构成，分数真实反映算法引擎相对于自身常态的运转质量。前 3 轮收敛期折线及折点以虚线标示；效率 >100% 时折点显示金色、<35% 时显示珊瑚红色以示警告。同维正负因子设有防振荡保护。效率 >100% 时强制显示正面因子，<35% 时强制显示负面因子——整体表现压倒个别波动。鼠标悬浮可查看真实数值及当轮主导因素。图表区域下方标注平均效率、彩色内存占用进度条和关键统计指标。

*A single unified freed-bytes accumulator feeds all displays. Per-cycle deltas are computed via cumulative differencing — independent of the lazily-updated system API — keeping logs, the status bar, and the chart in lockstep. The chart renders recent-cycle bars overlaid with the ERIS v5 efficiency line (0–100%+, uncapped). ERIS employs EWMA-adaptive baseline normalization across five dimensions — prediction accuracy, parameter convergence, side-effect control, learning convergence, and exploration completeness — with each dimension’s EWMA-decayed baseline serving as the 100% benchmark. Ratios pass through a soft sigmoid compressor (asymptotic cap 3.0), and the adaptive EWMA accelerates convergence when large deviations occur. The weighted sum yields an efficiency score that reflects the engine’s genuine performance relative to its own norm. Freed volume does not participate in scoring. Golden dots mark scores above 100%, coral-red dots warn below 35%. Anti-oscillation guards prevent the same dimension’s positive and negative labels from flip-flopping across consecutive cycles. When efficiency exceeds 100%, only positive factors are shown regardless of direction; when below 35%, only negative factors are shown — overall performance overrides individual dimensional fluctuations. The first 3 cycles display dashed lines and dots during convergence. Hover tooltips reveal the true value and dominant influencing factors. Below the chart: average efficiency, a color-coded memory bar, cumulative freed total, and key operation counters.*
