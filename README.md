...
2. **策略投票**（五树决策）：Kalman 预测收益、历史记忆验证、时序活跃度判断、Kalman 反事实优势、复合评分加权。总分 ≥ 0 即通过（不设硬性门槛）

3. **复合评分排序**：θ、Kalman 预期释放量、WS 大小、WS 回弹率四维加权，EFIS 可调权重

4. **PID 增益调度**：根据当前内存压力自动分三区调节响应强度——低压保守、中压标准、高压激进，避免单一参数在不同场景下顾此失彼

5. **并行执行**：4 线程池，按评分降序提交

5. **自适应 Pass 数**：大进程（WS>200 MB 或 θ 超阈值）→ 4 pass；默认 → 3 pass；低 θ → 1 pass

6. **PF 反馈**（先收后审）：对比清理前后缺页计数。释放量无条件计入统计，缺页仅用作 Thompson 学习信号的正负向判定
...
- **时间遗忘**：距离上次反馈超过 1 小时后，每小时向先验回归 3%，避免历史数据对当前状态的滞后影响
...
- **Kalman 预测树**：预期释放量 / 预期代价的比值，权重最高
- **历史记忆树**：同压力下的历史成功率，情景记忆验证
- **时序时机树**：基于 24 小时时间槽的活跃度判断
- **反事实预测树**：Kalman 预测的释放量差值——清理该进程相比最优替代进程的预期优势
- **复合评分树**：Kalman 置信度与 WS 趋势的综合加权
...
- **校准度**：对比 Kalman 预测与实际释放。偏差越大衰减越多——线性增大 Kalman 不确定性和缩归 Thompson 参数，替代旧版一次性重置
- **概念漂移**：双 EWMA 快慢速比检测进程行为突变。漂移时温和调整 Beta 和 Kalman 参数
- **探索覆盖**：统计从未被试探的进程比例，超过阈值时适度提高探索倾向
- **学习率自校准**：检查所有进程的预测误差，动态调整上下文权重学习率
...
**诊断方式**：每个参数配备专属的症状规则，基于滑动窗口内的释放效率、缺页速率、内存振幅等多维指标综合评估。症状须持续达到确认周期后才触发调整，防止单次噪声误调。参数调整步长经过约束，不会剧烈震荡。

**场景记忆**：支持多种场景独立参数记忆，场景切换时通过平滑插值过渡。场景由进程名关键词自动检测。

**持久化**：独立状态文件，加载时自动跳过已废弃参数，新参数缺失时取默认值。写入采用原子化操作，防止中途崩溃导致配置损坏。
...
**启动设置**：开机自启（快捷方式方式）、管理员权限启动（计划任务方式）、启动时自动开启守护、启动后最小化到托盘。

**关闭按钮行为**：最小化到托盘（守护继续运行）、直接退出程序、每次询问（默认）。

**清理操作**：7 种操作独立开关：ws（进程工作集）、standby（备用列表）、modified（已修改页）、compress（内存压缩）、volume（卷缓存）、registry（注册表缓存）、combine（内存合并）。前六项默认开启，内存合并默认关闭。

**清理模式**：下拉框选择 quick / normal / deep / full（默认 normal）。

**触发与日志**：紧急触发阈值（50-99%，默认 80%）、托盘左键行为、文件日志开关、进程清理深度（2-6 pass，默认 4）。
