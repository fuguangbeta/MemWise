...
| 系统文件缓存 | `filecache` | 开 | `SetSystemFileCacheSize` + `SystemFileCacheInformationEx` | 清空文件系统缓存，会降低文件操作速度直到重建 |
...
守护模式下的 gap fill 期间使用轻量管线（跳过系统级 WS 全清），只做 standby purge + modified flush，留给主 pass 在积累整个 gap 后一次性全量收割。主 pass 通过全量 8 步管线完成系统级 WS 全清 + Standby 全量清空，实现极致优化。

### 2.2 Layer2 — 进程级闲置页释放

对每个非自身进程调用 `EmptyWorkingSet`，释放其物理内存中的非活跃页。决策链路：

1. **`can_trim` 安全过滤**：排除系统核心进程（csrss、smss、wininit 等用户自定义黑名单中的进程）、前台窗口保护（中度压力 agg≥0.35 时即放开）、WS < 1 MB 的微小进程、WS 基线检查（上一轮清理后 WS 无增长则不重复清理）、失败冷却检查（PF 超标后进入 5 分钟冷却期）
...
6. **PF 反馈**（"先收后审"）：对比清理前后 PF 计数。释放量无条件计入统计，PF 仅用作 Thompson 学习信号的正负向判定
...
- **WS 回弹率筛选**：从 Layer2 未清理的进程中选出高回弹进程，追加一次清理
- **layer3_extra 追踪**：自动记录深度清理的净增量，用于 EFIS 判断 Layer3 的价值
...
while time.time() < deadline:
    gap fill 轻量压制:          # 轻量 standby purge + modified flush
        _layer1_memreduct(False) #   不碰 WS，留给 harvest
        fast-track 高回填修剪    #   高频 quick_retrim
        快照刷新+压力更新       #   实时读取系统内存状态
    harvest optimize(normal):   #   Layer2 进程修剪 + Layer1 轻量管线
    
# 末次
optimize(mode)                  #   全量 8 步管线 + Layer2 + Layer3
输出日志+图表
```

- **自适应 gap**：每轮后计算 `realse_per_proc`，与上一轮比对，决定 gap 缩放（8s 到 25s）。首次 gap = 15s
- **快车道**：`_layer2_process` 每轮将 `refill_ewma > 500KB/s` 的进程 PID 记录到 `_fast_track`；gap fill 期间消费 `_fast_track` 执行高频 `empty_ws`
- **轻量/全量分频**：gap fill 使用轻量模式维持压制，主 pass 使用全量 8 步实现深度收割
...
EFIS（Efficiency Feedback Intelligent System）覆盖全程序 5 层、9 参数的智能调参大脑。
...
| `target_usage` | 30% | 25-65% | PID 目标内存占用百分比 |
| `interval_high` | 10s | 5-20s | 高压时（agg>0.8）的日志/图表输出间隔 |
| `cooloff_base` | 300s | 60-360s | 失败冷却基数（实际冷却 = cooloff_base × 失败次数，最多×2） |
| `learning_rate` | 0.30 | 0.05-0.40 | 上下文特征权重的学习速度 |
| `composite_kalman_w` | 0.30 | 0.10-0.50 | 复合评分中 Kalman 分量 vs Thompson 分量的权重 |

**诊断方式**：每个参数配备专属的因果症状规则。如 `deepen_waste` 检查 2-pass 进程的平均额外释放量是否低于 10MB；`layer3_extra` 检查 Layer3 每轮平均释放是否低于 50MB；`pid_kp` 检查内存振幅和 PF 速率是否异常。症状持续 ≥2 周期后触发调参，防止噪声误调。

**场景记忆**：支持 game/browser/development/general 四个场景独立参数记忆。场景切换时 70% 当前参数 + 30% 场景历史参数平滑过渡，避免切换瞬间的参数跳跃。场景由进程名关键词自动检测。

**文件与加载**：独立文件 `efis_state.json`（与 learner 的 `memwise_state.json` 分离）。加载时自动跳过已废弃参数，新参数缺失时取默认值。`save()` 通过 `临时文件.write + os.replace` 原子化写入，防止写一半崩溃导致配置损坏。
...
激活后效果：非前台进程 WS 准入降至 2 MB，probe 间隔压缩到 0.15s，min_delta 降至 1 MB。前台（游戏）进程加强保护，全屏类名过滤排除 Chrome_WidgetWin_1、MozillaWindowClass、PPTFrameClass 等非游戏全屏窗口。
...
内存数据的采集优先使用 `NtQuerySystemInformation(SystemProcessInformation=5)` 一口返回所有进程的工作集和私有内存值，无需对每个进程执行 `OpenProcess`。此 API 可绕过某些安全软件的进程保护机制，确保全量覆盖。
...
**触发与日志**：紧急触发阈值（50-99%，默认 80%）、托盘左键行为（显示窗口/一键清理/无操作）、文件日志开关、进程清理深度（2-6 pass，默认 4）。
