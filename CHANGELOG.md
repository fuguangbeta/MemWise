...
> 优化引擎终极方案 + 全方位 Bug 修复与 UI 改进。针对清理量骤降、效率偏差、PF 误杀、trim 吞吐不足等实际运行问题，完成 6 大问题的终极方案实装。同时新增游戏模式天花板方案、自定义文件图标、线程池与时间预算重构，以及大量崩溃修复。累计修改 50+ 处代码，涉及 6 个核心模块。

### 🔧 系统操作 — 两阶段异步管线

将 `_layer1_system` 拆为三段：触发异步操作（零阻塞）→ 短暂等待 OS 处理 → 收割全部回收。`modified` 脏页写回提前到 standby 之前，同一轮即可回收脏页释放的待命内存。每轮系统操作释放量 +15~25%。

### 🔥 深度清理 — WS 回弹率选进程

移除原 `theta > 0.7` 硬门槛。改为按 WS 回弹率选择进程：`当前WS / 清理后WS >= 1.5` 直接 trim。深度清理不再空跑。

### ⏱ 全量 trim + 时间预算

`MAX_TRIM = 50` 硬上限已删除，全量候选提交。提交公式 `/3.0*2`→`/2.5*4`（适配 4 线程）。`_trim_budget` 从 `interval*0.4`→`0.55`。Trim 线程池 2→4。

### 🎯 自适应间隔

Probe 改为单次 empty_ws，一轮从 18s 降至 12s。sleep 0.3s→0.2s。`total_wait` 减半：1.0/1.5/2.0s，全量 trim 从 37.5s 降至 18.7s。

### 🛡 PF 阈值全面修正

`check_feedback` 新增 `passes` 参数 + `passes*40` 计入 `allowed_pf`。全部 `<`→`<=`。

### 📊 效率评分

overflow_bonus `0.25`→`0.30`。`total_attempts` 明确仅计 trim。

### 🎮 游戏模式终极方案

修复 `agg_threshold_fg = 0.8`（应为 0.6）导致游戏被跳过的问题。7 处改动覆盖前台保护门槛、全屏类名过滤、θ 门槛 0.10、WS 基线跳过、probe 0.15s、WS 准入 2MB、min_delta 1MB。

### 🐛 关键 Bug 修复

| 优先级 | 问题 | 修复 |
|:------:|------|------|
| P0 | 首轮清理暴跌至 300MB | 全量提交 + `/2.5*4` |
| P0 | `budgeted` 未定义崩溃 | 删除重复 fut 字典 |
| P0 | COM 初始化失败 | `S_FALSE`(1) 不再当错误 |
| P0 | `_cycle_trimmed` 未初始化 | 守护启动时初始化为 0 |
| P0 | `_compute_eris` 方法丢失 | 从 `_draw_chart` 提取 |
| P1 | Trim 失败偏高 | 传 passes + `passes*40` |
| P1 | PF 边界值误杀 | `<`→`<=` |
| P2 | EFIS 保存崩溃 | 重试 3 次 + 覆写保底 |
| P2 | 图表悬浮窗口不消失 | `bind("<Leave>")` |
| P2 | exe 图标为 Python 羽白 | 运行时导出 .ico + `--icon` |
| P2 | 子窗口图标黑色羽毛 | `SetClassLongPtrW` |
| P3 | tooltip 重复/图标冗余/单位写死 | 清理修复 |

### ⚙ 构建

- 全部 v1.4→v1.5
- `assets/icon.ico` 与运行时 `create_memwise_icon(32)` 逐字节一致
