...
> v3.5.14 发布后的全量维护增量：以局外审查者身份完成全项目 36 项问题清单逐一核实与修复，并在实施中额外发现修复 3 项（ERIS 冷启动中位数虚高、EFIS 释放量口径、启动 NameError 断点）；专项将长期闲置的 `learning_rate` 参数以最科学方式接入反馈学习主通道并配套双向诊断规则。共 47 项维护，回归测试 44 → 69 项全绿。
...
- 修复了游戏模式手动关闭后自动检测永久失效（与界面「自动检测或手动开启」语义不符）——手动关闭即恢复自动检测
