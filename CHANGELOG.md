...
### v1.2 — 学习系统初步成型 + 大量 Bug 修复

**修复 15 项 Bug**
- P0: Thompson θ 双定义导致上下文增强完全失效 → 删除重复定义
- P0: judger.py 缩进断裂导致死代码 → 重构缩进
- P1: empty_standby 首次调用双重执行 → 检测已执行直接返回
- P1: PID 控制器在 daemon 内外各更新一次(控制量翻倍) → optimize() 统一入口
- P1: 内存优先级映射反向(高θ进程拿到最高优先级反而被优先回收) → 低θ跳过、高θ设VERY_LOW
- P1: can_probe 不跳过前台进程 → 新增前台进程检查
- P2: can_probe 无 θ 过滤 → 样本数≥5 且 θ<0.2 时跳过
- P2: purge_expired 冷却时间+3600冗余 → 改为 now
- P2: CLI 与 GUI 清理路径不统一 → 统一走 cleaner.optimize()
- P2: 内存优先级永不刷新 → 每 30 tick 重新评估
- P3: 测量用快照用旧数据 → 实时读取 WS
- P3: normal 模式永不触发 Layer3 → aggressiveness≥0.6 时触发
- P3: except:pass 吞噬异常 → 改为打印到 stderr
- P3: ThreadPool 永不关闭 → 添加 shutdown()
- P3: state.json 路径硬编码 → 统一到 core.config

**学习系统**
- 上下文模型从 Probe 学习(record_probe 参与 _update_ctx_weights)
- 置信度改用 Beta 分布标准差(替代简单计数)
- Beta 分布时间衰减(每次 feed 乘衰减因子)
- 自适应学习率(置信度高小步、置信度低大步)
- 失败计数逐渐遗忘(衰减曲线替代硬重置)

**算法改进**
- 复合评分排序: θ×(0.5+0.3×置信度+0.2×ROI)
- 自适应清理轮数: WS<50MB→2轮、50~200MB→3轮、>200MB→4轮
- Standby 频率门控: 最小间隔 60s、可用内存>25%跳过重型清理
- 配置热加载: 守护模式每 60s 检查 config.yaml
- 全屏游戏检测: is_foreground_fullscreen
- 游戏名单扩充至 80+(新增国产/Steam/UWP/Xbox)
- 进程内存排行榜(树状显示+单击终止+悬浮高亮)
- 默认清理模式由 quick→normal
- state.json 过滤收紧: 30天/10样本→7天/5样本

**自启动优化**
- 新增 --minimized 参数(全程无闪烁)
- 自启动即守护(不再等 300ms)
- 统一追加 --minimized
...
- Thompson Sampling θ 缓存(避免重复计算)
- 进程画像自动清理(超30天)
- 四档颜色图标
- 守护模式完整分层清理(L1~L3)
- 操作过滤
- 30 进程上限
- 基础框架: Probe 机制、PID 控制器、守护模式、CLI 交互、系统托盘+热键

### v1.0 — 初始版本
- EmptyWorkingSet 进程清理
- CLI 界面(status/learn/optimize/daemon/profile)
- 基础配置文件
- 首次公开发布、核心架构建立
