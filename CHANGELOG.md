...
### v1.2
- **Thompson Sampling 学习系统**：Beta(α,β) 先验、α+=1/β+=1 更新、上下文 5 维特征修正
- **PID 控制器**：比例 0.8/积分 0.10/微分 0.15、目标 55%、连续反馈调节
- **游戏模式**：内置 80+ 游戏名单、全屏检测、CPU 门槛 2.0/联合概率 0.15
- **Standby 频率门控**：最小间隔 60s、可用内存>25%跳过重型清理
- **配置热加载**：守护模式每 60s 检查 config.yaml
- **进程排行榜**：GUI 树状显示+单击终止+悬浮高亮
- **自启动优化**：--minimized 静默、自启动即守护
- **state.json 过滤收紧**：30天/10样本→7天/5样本
- **修复 15 项 Bug**：含 P0 θ 双定义、P0 缩进断裂、P1 双重执行/PID 翻倍/优先级反向等

### v1.1
- 游戏模式初始版本(80+游戏名单)
- 双次清理(自适应间隔 300ms)
- 配置统一到 core/config.py
- Thompson Sampling θ 缓存
- 进程画像自动清理(超30天)
- 四档颜色图标(钢蓝/绿/黄/橙/红)
- 守护模式完整分层清理(L1~L3)
- 操作过滤、30进程上限
...
### v1.0
- EmptyWorkingSet 进程清理
- CLI 界面(status/learn/optimize/daemon)
- 基础配置文件
- 首次公开发布、核心架构建立
